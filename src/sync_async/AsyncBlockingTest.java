...
public class AsyncBlockingTest {
    ...
    static class WorkerB {
        ...
        CompletableFuture<Void> doWorkAndCallToALater(Consumer<Void> callBack) {
            return CompletableFuture.runAsync(() -> {
                ownJob.accept("I'm worker B. And I'm done my first job");
                callBack.accept(null);
                ownJob.accept("I'm worker B. And I'm done my second job");
            });
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // WorkerB가 일을 마치지 않아쓴데 메인 스레드가 종료되는 경우 애플리케이션이 종료되므로 이런 현상을 방지하는 코드 추가
        joinPoint.join();
